            ...
            public void onComplete(@NonNull Task<AuthResult> task) {
                if(task.isSuccessful())
                {
                   ...
                   final String full_name_of_user=getIntent().getStringExtra("fullname_str");
                    ...
                    HelperClass helperclass=new HelperClass(full_name_of_user,amizone_id_of_user,phone_num_of_user,email_id_of_user);
                    referenceUser.child(firebaseAuth.getUid()).setValue(helperclass, new DatabaseReference.CompletionListener() {
                        @Override
                        public void onComplete(@Nullable DatabaseError databaseError, @NonNull DatabaseReference databaseReference) {
                            if (databaseError != null) {
                                progress_bar_otp.setVisibility(View.GONE);
                                Toast.makeText(Otp_verification.this, "Registraion Failed", Toast.LENGTH_SHORT).show();
                            } else {
                                Toast.makeText(Otp_verification.this, "Successfully Registered", Toast.LENGTH_SHORT).show();
                                Intent i = new Intent(Otp_verification.this, Succesfully_sign_up.class);
                                i.putExtra("fullname",full_name_of_user);
                                i.putExtra("email_id",email_id_of_user);
                                i.putExtra("amizone_id",amizone_id_of_user);
                                startActivity(i);
